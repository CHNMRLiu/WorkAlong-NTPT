<template>
  <div class="page">
    <PageHeader title="计量环比" subtitle="同一表计本期与上期的能耗指标环比变化" />

    <el-card class="panel">
      <el-form :inline="true" @submit.prevent>
        <el-form-item label="表计" required>
          <el-select v-model="meterId" filterable placeholder="选择表计" style="width:180px">
...
import { listMeters, ratioEnergy } from '@/api'
...
async function run() {
  if (!meterId.value) return ElMessage.warning('请选择表计')
  ...
  const r = await ratioEnergy({
    target_id: meterId.value, dimension: dimension.value, mode: 'meter',
